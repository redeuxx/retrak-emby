...
```bash
dotnet build Trakt.sln -c Release
```
This produces a `Trakt.dll` file in the build output directory (`Trakt/bin/Release/`).

### 2. Install the Plugin
1. Copy the compiled `Trakt.dll` file.
...
4. To configure custom OAuth clients, replace the client credentials in `TraktURIs.cs` and `configPage.html` with your registered application credentials.
