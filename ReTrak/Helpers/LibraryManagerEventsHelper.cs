...
namespace ReTrak.Helpers
{
    internal class LibraryManagerEventsHelper
    {
        ...
        public LibraryManagerEventsHelper(ILogger logger, ReTrakApi retrakApi)
        {
        ...
        public void QueueItem(BaseItem item, EventType eventType)
        {
            ...
            var users = Plugin.Instance.PluginConfiguration.ReTrakUsers;

            if (users == null || users.Length == 0) return;

            // we need to process the video for each user
            foreach (var user in users.Where(x => _retrakApi.CanSync(item, x)))
            {
                ...
                var libraryEvent = new LibraryEvent { Item = item, ReTrakUser = user, EventType = eventType };
        ...
        private async Task OnQueueTimerCallbackInternal()
        {
            ...
            _queuedEvents.Clear();
            foreach (var retrakUser in Plugin.Instance.PluginConfiguration.ReTrakUsers)
            {
                var queuedMovieDeletes = queue.Where(ev =>
                    new Guid(ev.ReTrakUser.LinkedMbUserId).Equals(new Guid(retrakUser.LinkedMbUserId)) &&
                    ev.Item is Movie &&
                ...
                if (queuedMovieDeletes.Any())
                {
                    ...
                    await ProcessQueuedMovieEvents(queuedMovieDeletes, retrakUser, EventType.Remove, CancellationToken.None).ConfigureAwait(false);
                }
                else
                {
                ...
                if (queuedMovieAdds.Any())
                {
                    ...
                    await ProcessQueuedMovieEvents(queuedMovieAdds, retrakUser, EventType.Add, CancellationToken.None).ConfigureAwait(false);
                }
                else
                {
                ...
                if (queuedEpisodeDeletes.Any())
                {
                    ...
                    await ProcessQueuedEpisodeEvents(queuedEpisodeDeletes, retrakUser, EventType.Remove, CancellationToken.None).ConfigureAwait(false);
                }
                else
                {
                ...
                if (queuedEpisodeAdds.Any())
                {
                    ...
                    await ProcessQueuedEpisodeEvents(queuedEpisodeAdds, retrakUser, EventType.Add, CancellationToken.None).ConfigureAwait(false);
                }
                else
                {
                ...
                if (queuedShowDeletes.Any())
                {
                    ...
                    await ProcessQueuedShowEvents(queuedShowDeletes, retrakUser, EventType.Remove, CancellationToken.None).ConfigureAwait(false);
                }
                else
                {
        ...
        private async Task ProcessQueuedShowEvents(IEnumerable<LibraryEvent> events, ReTrakUser retrakUser, EventType eventType, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                foreach (var show in shows)
                    await _retrakApi.SendLibraryUpdateAsync(show, retrakUser, cancellationToken, eventType).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
        ...
        /// <param name="eventType"></param>
        /// <returns></returns>
        private async Task ProcessQueuedMovieEvents(IEnumerable<LibraryEvent> events, ReTrakUser retrakUser, EventType eventType, CancellationToken cancellationToken)
        {
            ...
            try
            {
                await _retrakApi.SendLibraryUpdateAsync(movies, retrakUser, cancellationToken, eventType).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
        ...
        /// <param name="eventType"></param>
        /// <returns></returns>
        private async Task ProcessQueuedEpisodeEvents(IEnumerable<LibraryEvent> events, ReTrakUser retrakUser, EventType eventType, CancellationToken cancellationToken)
        {
            ...
            foreach (var ep in episodes)
            {
                ...
                {
                    // We're starting a new series. Time to send the current one to retrak.tv
                    await _retrakApi.SendLibraryUpdateAsync(payload, retrakUser, cancellationToken, eventType);
